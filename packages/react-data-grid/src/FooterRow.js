...
const BaseHeaderCell        = require('./FooterCell');
...
const FooterRow = React.createClass({
  propTypes: {
    width: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
  ...
  getSortableHeaderCell(column) {
    let sortDirection = (this.props.sortColumn === column.key) ? this.props.sortDirection : SortableFooterCell.DEFINE_SORT.NONE;
    return <SortableFooterCell columnKey={column.key} onSort={this.props.onSort} sortDirection={sortDirection}/>;
  },
  ...
  getCells(): Array<HeaderCell> {
    ...
    for (let i = 0, len = this.getSize(this.props.columns); i < len; i++) {
      ...
      let HeaderCell = BaseHeaderCell;//column.draggable ? this.props.draggableHeaderCell : BaseHeaderCell;
      let cell = (
        <HeaderCell
